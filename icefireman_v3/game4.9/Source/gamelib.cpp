...
void CAnimation::OnShow(int num)
{
	GAME_ASSERT(bmp.size() != 0, "CAnimation: Bitmaps must be loaded before they are shown.");
	bmp_iter->SetTopLeft(x, y);
	bmp_iter->ShowBitmap(num);
}
...
void CInteger::LoadBitmap()
{
    ...
    //
    if (!isBmpLoaded) {
        int di[11]= {number0,number1,number2,number3,number4,number5,number6,number7,number8,number9,IDB_MINUS};

        for (int i=0; i < 11; i++)
            digit[i].LoadBitmap(di[i],RGB(255,255,255));
    ...
    : NUM_GAME_STATES(5)
{
	...
	gameStateTable[GAME_STATE_ATTACK] = new CGameStateAttack(this);
	gameStateTable[GAME_STATE_NARUTO] = new CGameStateNaruto(this);
    gameState = NULL;
}
